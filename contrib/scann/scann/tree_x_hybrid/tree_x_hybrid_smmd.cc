...
TreeXHybridSMMD<T>::TreeXHybridSMMD(
    ...
    : SingleMachineSearcherBase<T>(dataset, hashed_dataset,
                                   hashed_dataset_by_token,
                                   hashed_dataset_packed,
...
Status TreeXHybridSMMD<T>::BuildLeafSearchers(
  ...
  const DenseDataset<uint8_t>* hashed_dataset_by_token = this->hashed_dataset_by_token().get();
  const DatapointIndex n_tokens = datapoints_by_token.size();
  leaf_searchers_.resize(n_tokens);
  DatapointIndex num_data = 0;
  DatapointIndex num_packed_data = 0;
  for (int32_t token = 0; token < n_tokens; ++token) {
    const absl::Time token_start = absl::Now();
    auto leaf_size = datapoints_by_token[token].size();
    auto leaf_packed_size = (leaf_size + 31) & (~31);
    shared_ptr<DenseDataset<uint8_t>> leaf_hashed_dataset_packed = nullptr;
    if (this->hashed_dataset_packed()) {
      auto storage = DenseDataWrapper<uint8_t>(
        const_cast<uint8_t*>(&this->hashed_dataset_packed()->data(num_packed_data)[0]),
        leaf_packed_size * this->hashed_dataset_packed()->dimensionality());
      leaf_hashed_dataset_packed = std::make_shared<DenseDataset<uint8_t>>(
        std::move(storage), leaf_packed_size);
    }
    if (!hashed_dataset && !hashed_dataset_by_token) {
      // generate hashed dataset on the fly in building phase
      shared_ptr<TypedDataset<T>> dataset_partition(
          PartitionDataset(*dataset, datapoints_by_token[token]));
      TF_ASSIGN_OR_RETURN(
          unique_ptr<SingleMachineSearcherBase<T>> leaf_searcher,
          leaf_searcher_builder(dataset_partition, nullptr,
            leaf_hashed_dataset_packed, token));
      ...
      shared_ptr<DenseDataset<uint8_t>> hashed_dataset_partition = nullptr;
      if (hashed_dataset_by_token) {
        // generate from hashed_dataset_by_token
        auto data_dim = hashed_dataset_by_token->dimensionality();
        auto part_begin = hashed_dataset_by_token->data().begin() + num_data * data_dim;
        absl::Span<uint8_t> part_data(const_cast<uint8_t*>(part_begin), leaf_size * data_dim);
        hashed_dataset_partition = std::make_shared<DenseDataset<uint8_t>>(part_data, leaf_size);
        hashed_dataset_partition->set_packing_strategy(hashed_dataset_by_token->packing_strategy());
        hashed_dataset_partition->set_dimensionality(hashed_dataset_by_token->dimensionality());
        hashed_dataset_partition->hash_4bit = hashed_dataset_by_token->hash_4bit;
      }
      else {
        // generate from hashed_dataset
        hashed_dataset_partition = std::shared_ptr<DenseDataset<uint8_t>>(
          down_cast<DenseDataset<uint8_t>*>(
            PartitionDataset(*hashed_dataset, datapoints_by_token[token])));
      }
      TF_ASSIGN_OR_RETURN(
          unique_ptr<SingleMachineSearcherBase<T>> leaf_searcher,
          leaf_searcher_builder(nullptr, hashed_dataset_partition, leaf_hashed_dataset_packed, token));
    ...
    num_data += leaf_size;
    num_packed_data += leaf_packed_size;
...
Status TreeXHybridSMMD<T>::FindNeighborsImpl(const DatapointPtr<T>& query,
  ...
  if (PreTokenizationEnabled(tree_x_params)) {
    ...
    if (tree_x_params) {
      const auto max_spilling_config =
          tree_x_params->max_spilling_config();
      if (max_spilling_config.valid()) {
        ...
        SCANN_RETURN_IF_ERROR(
            kmeans_tokenizer->TokensForDatapointWithSpillingAndOverride(
                query, max_spilling_config, &query_tokens_storage));
        override = true;
      }
    }
  ...
  if (params.pre_reordering_crowding_enabled()) {
    ret = FailedPreconditionError("Crowding is not supported.");
  } else {
    ret = FindNeighborsPreTokenizedImpl(
        query, params, query_tokens,
        TopNeighbors<float>(params.pre_reordering_num_neighbors()), result);
  }
  auto t2 = std::chrono::high_resolution_clock::now();
  std::chrono::duration<float, std::micro> dt0 = t1 - t0;
  std::chrono::duration<float, std::micro> dt1 = t2 - t1;
  if (VLOG_IS_ON(1)) {
    LOG_EVERY_N(INFO, 100)
      << "TreeXHybridSMMD<T>::FindNeighborsImpl"
      << " find_tokens=" << size_t(dt0.count()) / 1000.0f
      << " find_neighbors=" << size_t(dt1.count()) / 1000.0f
      << " query_tokens.size=" << query_tokens.size();
  }
  return ret;
}
...
Status TreeXHybridSMMD<T>::FindNeighborsBatchedImpl(
  ...
  if (pre_tokenized) {
    ...
    vector<MaxSpillingConfig> max_spilling_configs(queries.size());
    bool override = false;
    ...
    for (int i = 0; i < queries.size(); ++i) {
      auto tree_x_params =
          params[i]
              .searcher_specific_optional_parameters<TreeXOptionalParameters>();
      if (tree_x_params)
        max_spilling_configs[i] = tree_x_params->max_spilling_config();
      if (max_spilling_configs[i].valid()) override = true;
    }
...
Status TreeXHybridSMMD<T>::FindNeighborsPreTokenizedBatchedOptimizedImpl(
  ...
  for (auto [leaf_idx, query_idxs] : Enumerate(queries_by_partition)) {
    ...
    if (!query_idxs.empty()) non_empty_leaf_tokens.emplace_back(leaf_idx);
  }
  ...
  std::unordered_map<std::thread::id, ThreadState> thread_states;
  std::mutex mutex;

  QueryParallelFor<600>(Seq(non_empty_leaf_tokens.size()), [&](size_t ind) {
    auto leaf_idx = non_empty_leaf_tokens[ind];
    auto& query_idxs = queries_by_partition[leaf_idx];
    CHECK(!query_idxs.empty());

    ThreadState* thread_state = nullptr;
    {
      std::lock_guard<std::mutex> lock(mutex);
      auto tid = std::this_thread::get_id();
      if (thread_states.count(tid) == 0) {
        // initialize per thread state
        auto& r = thread_states[tid];
        r.leaf_results.reserve(max_queries_per_partition);
        r.backing_storage.reserve(queries.dimensionality() * max_queries_per_partition);
      }
      thread_state = &thread_states[tid];
    }
    auto &leaf_results = thread_state->leaf_results;
    auto &backing_storage = thread_state->backing_storage;
    ...
    auto leaf_dataset = DenseDataset<T>(std::move(backing_storage), query_idxs.size());
    ...
    auto status = leaf_searchers_[leaf_idx]->FindNeighborsBatchedNoSortNoExactReorder(
    ...
    // update the results
    {
      ...
      for (auto [local_query_idx, global_query_idx] : Enumerate(query_idxs)) {
        CHECK_LT(local_query_idx, leaf_results.size());
...
Status TreeXHybridSMMD<T>::FindNeighborsPreTokenizedImpl(
  ...
  // `&& false`: turn off this execution path to avoid filter error
  if (params.restricts_enabled() && !PreTokenizationEnabled(params) &&
      ...
      params.restrict_whitelist()->NumPointsWhitelisted() <=
          params.pre_reordering_num_neighbors() && false) {
...
TreeXHybridSMMD<T>::ExtractSingleMachineFactoryOptions() {
  ...
  // leaf_opts will have the updated hashed_dataset_by_token
  TF_ASSIGN_OR_RETURN(
      SingleMachineFactoryOptions leaf_opts,
      MergeAHLeafOptions(leaf_searchers_, datapoints_by_token_,
                         this->hashed_dataset_by_token(), dataset_size));
...
Status TreeXHybridSMMD<T>::PreprocessQueryIntoParamsUnlocked(
  ...
  if (params) {
    ...
    auto max_spilling_config = params->max_spilling_config();
    SCANN_RETURN_IF_ERROR(
        kmeans_tokenizer->TokensForDatapointWithSpillingAndOverride(
            query, max_spilling_config, &centers_to_search));
  } else {
    SCANN_RETURN_IF_ERROR(query_tokenizer_->TokensForDatapointWithSpilling(
