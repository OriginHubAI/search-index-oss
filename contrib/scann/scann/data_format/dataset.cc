...
template <typename T>
DenseDataset<T>::DenseDataset(Storage datapoint_vec,
...
template <typename T>
DenseDataset<T>::DenseDataset(Storage datapoint_vec, size_t num_dp)
    : DenseDataset<T>(
...
#ifdef MYSCALE_MODE
  checkAvailableMemory(n * stride_ * sizeof(T));
#endif
