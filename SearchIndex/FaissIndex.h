...
template <typename IS, typename OS, IDSelector IDS>
class MSTGBinaryIndex : public FaissIndex<IS, OS, IDS, DataType::BinaryVector>
{
public:
    using SearchParamsPtr = std::shared_ptr<faiss::SearchParameters>;
    MSTGBinaryIndex(
        ...
        size_t max_points_,
        Parameters params) :
        FaissIndex<IS, OS, IDS, DataType::BinaryVector>(
            name_, IndexType::BinaryIVF, metric_, data_dim_, max_points_)
    {
        if (max_points_ < 1000 || max_points_ * data_dim_ < 16000)
        {
            SI_THROW_FMT(
                ErrorCode::BAD_ARGUMENTS,
                "max_points=%lu data_dim=%lu too small for BinaryMSTG, "
                "try BinaryFLAT Index instead",
                max_points_,
                data_dim_);
        }

        ncentroids = static_cast<size_t>(sqrt(static_cast<float>(max_points_)));
        Parameters faiss_params;
        faiss_params.setParam("ncentroids", ncentroids);
        ivf_binary = std::make_shared<
            FaissIVFIndex<IS, OS, IDS, DataType::BinaryVector>>(
            name_,
            IndexType::BinaryIVF,
            metric_,
            data_dim_,
            max_points_,
            faiss_params);
        this->setFaissIndex(ivf_binary->getFaissBinaryIndex());
        this->extractAndSetCommonIndexParams(params);
        raiseErrorOnUnknownParams(params);
    }
    ...
    extractSearchParams(Parameters & params, IDS * /*filter*/) override
    {
        float alpha = params.extractParam("alpha", 3);
        SI_THROW_IF_NOT_MSG(
            alpha >= 1.0 && alpha <= 4.0,
            ErrorCode::BAD_ARGUMENTS,
        ...
        raiseErrorOnUnknownParams(params);
        auto * ivf_params = new faiss::IVFSearchParameters;
        ivf_params->nprobe = std::exp(alpha * 0.6) * 0.008 * ncentroids;
        return SearchParamsPtr(ivf_params);
    }

    virtual IndexResourceUsage getResourceUsage() const final
    {
        return ivf_binary->getResourceUsage();
    }

    std::shared_ptr<FaissIVFIndex<IS, OS, IDS, DataType::BinaryVector>>
        ivf_binary;
    size_t ncentroids;
};
